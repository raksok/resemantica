...
| `locked_glossary` | `glossary_entry_id` | `release_id`, `source_term`, `target_term`, `category`; UNIQUE `(release_id, normalized_source_term, category)` |
...
Locked glossary uniqueness is source-based. Different Chinese source terms may
share the same English target term, even within one category.
