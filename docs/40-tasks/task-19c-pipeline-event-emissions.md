...
| `preprocess-glossary.resolve.started/completed` | Saved glossary vote replay without model calls |
| `preprocess-glossary.resolve.unresolved` | Saved glossary votes did not resolve |
| `preprocess-glossary.resolve.snapshot.artifact_written` | Resolve-only candidate snapshot written |
| `preprocess-glossary.resolve.failed` | Resolve-only replay failed |
| `preprocess-glossary.fill.started/completed` | Optional unresolved-vote filler pass |
| `preprocess-glossary.fill.loading_completed` | Filler candidate selection completed |
| `preprocess-glossary.fill.model_started/completed` | Per filler model missing-vote batch |
| `preprocess-glossary.fill.resolution.started/completed` | Filler-assisted deterministic resolution |
| `preprocess-glossary.fill.unresolved` | Filler-assisted votes still did not resolve |
| `preprocess-glossary.fill.failed` | Filler vote generation or resolution failed |
| `preprocess-glossary.promote.started/completed` | Promotion phase |
| `preprocess-glossary.completed` | Pipeline ends |

Glossary discovery keeps legacy `filter_completed`, `dedup_started`, `dedup_completed`, and `preprocess-glossary.eval.eval_batch_*` events for compatibility while adding scoped discover events for current operators. Glossary translation keeps model-first vote generation order and skips existing votes for the same release, run, and model unless forced. Interrupted reruns may use `vote_resume` loading when prior run telemetry and complete model votes prove the pending set; in that mode, `loading_completed` means the resume ID universe is loaded, while candidate rows are hydrated later in `candidate_id` primary-key chunks without release-wide candidate scans. Chapter `started`/`completed` events are emitted when resolution and canonical saves begin and finish for a chapter, after all model vote batches complete. `glossary-resolve` uses the `resolve.*` family because it replays saved votes and writes a candidate snapshot without model work. `glossary-fill` uses the `fill.*` family because it performs targeted filler model calls and database/vote-status updates without writing a candidate snapshot.
