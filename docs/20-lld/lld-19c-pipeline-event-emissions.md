...
For glossary translation, chapter events belong to the resolution/save phase. Model vote generation is model-first and emits per-model start/completion events before resolution begins. Reruns skip existing votes for the same release, run, and model unless forced. If prior run telemetry and complete model votes prove the pending set, loading may use `vote_resume` to avoid a full pending-candidate scan.

`loading_started` marks DB preparation and pending-candidate loading before model work begins. `loading_completed` reports the selected `load_strategy`, the loaded pending count, and elapsed load timings. `model_completed.skipped_count` is the number of loaded candidates skipped for that model because durable votes already existed for `(release_id, run_id, model_name)`. A `preprocess-glossary.translate.failed` event after a model-server crash, including llama.cpp-backed local model crashes, records the failed phase/model/candidate but does not imply saved votes were rolled back or discarded. Operators can rerun the same release, run, and config without `--force` to continue from missing votes.
