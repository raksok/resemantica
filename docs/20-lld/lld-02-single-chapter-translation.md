...
- Mixed-language Pass 1 candidates remain invalid, but their valid English content is supplied to the next same-model correction request together with the exact remaining Chinese spans. This recovery uses the existing content-attempt budget and does not change the base prompt version or invalidate successful cached blocks.
