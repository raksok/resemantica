...
10. Retry Pass 2 block validation failures up to `[translation].pass2_validation_retries` additional attempts before failing the chapter. Retries cover structural, restoration, fidelity, and untranslated-Chinese validation failures; prompt budget failures are not retried here. Chinese remaining in a draft is always a fidelity error, even when the model reports no other fidelity issue. After retries, any remaining Chinese span fails the block with exact-span diagnostics.
