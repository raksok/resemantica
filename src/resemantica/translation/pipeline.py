    ...
    try:
        ...
        if (
            ...
            for record in records:
                ...
                structure = validate_structure(source_text, draft_text)
                ...
                if structure.is_valid:
                    ...
                    if blocking_restore_warnings:
                        ...
                        structure = validate_structure(source_text, "")
                    else:
                        pass1_blocks.append(
                            {
                                "block_id": block_id,
                ...
                if _placeholder_tokens(source_text):
                    pass1_blocks.append(
                        {
                            "block_id": block_id,
                            "parent_block_id": parent_block_id,
                ...
                retry_segments = _split_for_retry(source_text, max_chars=750)
                ...
                for segment_index, segment_source in enumerate(retry_segments, start=1):
                    ...
                    segment_structure = validate_structure(segment_source, segment_draft)
                    pass1_structure_checks.append(
                        {
            ...
            if pass1_failed:
                _write_json(
                    structure_report_path,
                    {
                        ...
                        "status": "failed",
                        "checks": pass1_structure_checks,
                    },
                )
                raise RuntimeError("Pass 1 structural validation failed.")

        pass2_blocks: list[dict[str, Any]] = []
        pass2_structure_checks: list[dict[str, Any]] = []
        ...
        structure_report = {
            ...
            "validation_type": "structural",
            "status": "failed"
            if any(check["status"] == "failed" for check in pass1_structure_checks + pass2_structure_checks)
            else "success",
            "checks": pass1_structure_checks + pass2_structure_checks,
        }
        fidelity_report = {
            "release_id": release_id,
            "run_id": run_id,
